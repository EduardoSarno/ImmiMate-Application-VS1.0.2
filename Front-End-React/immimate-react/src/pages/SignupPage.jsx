  ...
  useEffect(() => {
    ...
    if (token && email) {
      ...
      setTimeout(async () => {
        ...
        Logger.info(`SignupPage: User has profile? ${hasProfile}`);
        
        if (hasProfile) {
          Logger.info("SignupPage: User has a profile, redirecting to dashboard");
          navigate('/dashboard');
        } else {
          Logger.info("SignupPage: User doesn't have a profile, redirecting to form");
          navigate('/form');
        }
      }, 1500);
    }
